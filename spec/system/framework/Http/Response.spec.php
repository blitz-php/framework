...
describe('Response', function () {
	describe('Status code', function () {
        ...
        it('Status code leve une erreur lorsque le code est invalide', function () {
			...
			expect(fn() => $response->withStatus(54322))->toThrow(new HttpException());
        });
        ...
        it('Erreur lorsque le statut code est inconnue', function () {
			...
			expect(fn() => $response->withStatus(115))->toThrow(new HttpException(lang('HTTP.unknownStatusCode', [115])));
        });

        it('Erreur lorsque le statut code est petit', function () {
			$response = new Response();

			expect(fn() => $response->withStatus(95))->toThrow(new HttpException(lang('HTTP.invalidStatusCode', [95])));
        });

        it('Erreur lorsque le statut code est grand', function () {
			$response = new Response();

			expect(fn() => $response->withStatus(695))->toThrow(new HttpException(lang('HTTP.invalidStatusCode', [695])));
        });
	...
	describe('JSON et XML', function() {
		it('JSON avec un tableau', function() {
			...
			expect($response->getHeaderLine('content-type'))->toMatch(fn($content) => str_contains($content, 'application/json'));
		});
		...
		it('XML avec un tableau', function() {
			...
			expect($response->content())->toBe($expected);
			expect($response->getHeaderLine('content-type'))->toMatch(fn($content) => str_contains($content, 'application/xml'));
		});
	});
	...
	describe('Telechargement', function() {
		it('StreamDownload', function() {
			...
			expect(fn() => $emitter($actual, 8192))->toEcho('data');
		});
		...
		it('Download', function() {
			...
			expect(fn() => $emitter($actual, 8192))->toEcho(file_get_contents(__FILE__));
		});

		it('Download avec un fichier innexistant', function() {
			$response = new Response();

			expect(fn() => $response->download('__FILE__'))->toThrow(new LoadException());
		});
	});
	...
	describe('With', function() {
		...
		it('withCache', function() {
			...
			expect($response->getHeaderLine('Cache-Control'))->toMatch(fn($value) => str_contains($value, 'public, max-age='));
		});
	});
});
