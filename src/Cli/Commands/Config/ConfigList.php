...
class ConfigList extends Command
{
    ...
    public function handle()
    {
        ...
        if ($onlyAvailable) {
            $filteredConfigs = array_filter($filteredConfigs, static fn ($c) => ! $c['published']);
        }
        if ($onlyPublished) {
            $filteredConfigs = array_filter($filteredConfigs, static fn ($c) => $c['published']);
        }
        if ($namespaceFilter) {
            $filteredConfigs = array_filter($filteredConfigs, static fn ($c) => str_contains(strtolower($c['namespace']), strtolower($namespaceFilter)));
        }
    ...
    {
        $total     = count($configs);
        $published = count(array_filter($configs, static fn ($c) => $c['published']));
